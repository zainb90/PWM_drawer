 ...
 * PWM_Program.c
 *
 * Created: 2/8/2025 5:26:47 PM
 *  Author: ahmad
 */ 
#include "PWM_Configuration.h"

uint16_t gu16_PWM_TimerTopValue = 0U;
...
void PWM_SetDuty(PWM_ChannelType Tmr_Channel, PWM_OutputStateType State, uint16_t Duty_Percent) {
	switch (Tmr_Channel) {
		...
		switch (State) {
			case PWM_NON_INVERTED: *TIMER0_CMP_REG = (uint8_t)(Duty_Percent * PWM_TIMER_0_RESOLUTION / 100.0); break;
			case PWM_INVERTED: *TIMER0_CMP_REG = (uint8_t)((100.0 - Duty_Percent) * PWM_TIMER_0_RESOLUTION / 100.0); break;
			default: break;
		}
		break;
		case PWM_TIMER_1_CHANNEL_A:
		switch (State) {
			case PWM_NON_INVERTED: *(uint16_t*)TIMER1_CMP_A_REG_L = (uint16_t)(Duty_Percent * gu16_PWM_TimerTopValue / 100.0); break;
			case PWM_INVERTED: *(uint16_t*)TIMER1_CMP_A_REG_L = (uint16_t)((100.0 - Duty_Percent) * gu16_PWM_TimerTopValue / 100.0); break;
			default: break;
		}
		break;
		case PWM_TIMER_1_CHANNEL_B:
		switch (State) {
			case PWM_NON_INVERTED: *(uint16_t*)TIMER1_CMP_B_REG_L = (uint16_t)(Duty_Percent * gu16_PWM_TimerTopValue / 100.0); break;
			case PWM_INVERTED: *(uint16_t*)TIMER1_CMP_B_REG_L = (uint16_t)((100.0 - Duty_Percent) * gu16_PWM_TimerTopValue / 100.0); break;
			default: break;
		}
		break;
		case PWM_TIMER_2_CHANNEL:
		switch (State) {
			case PWM_NON_INVERTED: *TIMER2_CMP_REG = (uint8_t)(Duty_Percent * PWM_TIMER_2_RESOLUTION / 100.0); break;
			case PWM_INVERTED: *TIMER2_CMP_REG = (uint8_t)((100.0 - Duty_Percent) * PWM_TIMER_2_RESOLUTION / 100.0); break;
			default: break;
		}
		break;
		default: break;
