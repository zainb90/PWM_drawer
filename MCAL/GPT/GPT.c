...
void GPT_Init(const GPT_ConfigurationType* Config_Arr_Ptr) {
	for (uint8_t i = 0; i < NUM_TIMERS_OPERATING; i++, Config_Arr_Ptr++) {
...
void GPT_Delay_us(const GPT_ConfigurationType* Config_Ptr, uint16_t Delay_Time_us) {
	// Delay_Time_us = 10^6 * Prescale * Num_Counts / F_CPU
	uint16_t Tmr_Counts = 0U;
	switch (Config_Ptr->Clk_Prescale) {
		case GPT_PRESCALE_1:	Tmr_Counts = (uint16_t)(Delay_Time_us * F_CPU / (1.0 * 1000000UL)); break;
		case GPT_PRESCALE_8:	Tmr_Counts = (uint16_t)(Delay_Time_us * F_CPU / (8.0 * 1000000UL)); break;
		case GPT_PRESCALE_32:	Tmr_Counts = (uint16_t)(Delay_Time_us * F_CPU / (32.0 * 1000000UL)); break;
		case GPT_PRESCALE_64:	Tmr_Counts = (uint16_t)(Delay_Time_us * F_CPU / (64.0 * 1000000UL)); break;
		case GPT_PRESCALE_128:	Tmr_Counts = (uint16_t)(Delay_Time_us * F_CPU / (128.0 * 1000000UL)); break;
		case GPT_PRESCALE_256:	Tmr_Counts = (uint16_t)(Delay_Time_us * F_CPU / (256.0 * 1000000UL)); break;
		case GPT_PRESCALE_1024: Tmr_Counts = (uint16_t)(Delay_Time_us * F_CPU / (1024.0 * 1000000UL)); break;
		default:break;
	}
	switch (Config_Ptr->Tmr_Channel) {
		...
		*TIMERS_INT_MASK_REG &= (0xFCU);
		switch (Config_Ptr->Op_Mode) {
			case GPT_CTC_MODE:
			*TIMER0_CMP_REG = (uint8_t)Tmr_Counts;
			...
			*TIMER0_CNTR_REG = 256-(uint8_t)(Tmr_Counts);
		...
		*TIMERS_INT_MASK_REG &= (0xC3U);
		switch (Config_Ptr->Op_Mode) {
			case GPT_CTC_MODE:
			*TIMER1_CMP_A_REG_L = (uint8_t)(Tmr_Counts & 0x00FFU);
			*TIMER1_CMP_A_REG_H = (uint8_t)((Tmr_Counts & 0xFF00U)>>8);
			...
			*TIMERS_INT_MASK_REG |= ((Config_Ptr->Int_State)<<TIMER1_CMP_A_INT_BIT);
			return;
			case GPT_NORMAL_MODE:
			*TIMER1_CNTR_REG_L = (uint8_t)((65535-Tmr_Counts) & 0x00FFU);
			*TIMER1_CNTR_REG_H = (uint8_t)((65535-Tmr_Counts)>>8);
		...
		*TIMERS_INT_MASK_REG &= (0x3FU);
		switch (Config_Ptr->Op_Mode) {
			case GPT_CTC_MODE:
			*TIMER2_CMP_REG = (uint8_t)Tmr_Counts;
			...
			*TIMERS_INT_MASK_REG |= ((Config_Ptr->Int_State)<<TIMER2_CMP_INT_BIT);
			return;
			case GPT_NORMAL_MODE:
			*TIMER2_CNTR_REG = 256-(uint8_t)(Tmr_Counts);
...
void GPT_Delay_ms(const GPT_ConfigurationType* Config_Ptr, uint16_t Delay_Time_ms) {
	// Delay_Time_ms = 10^3 * Prescale * Num_Counts / F_CPU
	uint16_t Tmr_Counts = 0U;
	switch (Config_Ptr->Clk_Prescale) {
		case GPT_PRESCALE_1:	Tmr_Counts = (uint16_t)(Delay_Time_ms * F_CPU / (1.0 * 1000UL)); break;
		case GPT_PRESCALE_8:	Tmr_Counts = (uint16_t)(Delay_Time_ms * F_CPU / (8.0 * 1000UL)); break;
		case GPT_PRESCALE_32:	Tmr_Counts = (uint16_t)(Delay_Time_ms * F_CPU / (32.0 * 1000UL)); break;
		case GPT_PRESCALE_64:	Tmr_Counts = (uint16_t)(Delay_Time_ms * F_CPU / (64.0 * 1000UL)); break;
		case GPT_PRESCALE_128:	Tmr_Counts = (uint16_t)(Delay_Time_ms * F_CPU / (128.0 * 1000UL)); break;
		case GPT_PRESCALE_256:	Tmr_Counts = (uint16_t)(Delay_Time_ms * F_CPU / (256.0 * 1000UL)); break;
		case GPT_PRESCALE_1024: Tmr_Counts = (uint16_t)(Delay_Time_ms * F_CPU / (1024.0 * 1000UL)); break;
		default:break;
	}
	switch (Config_Ptr->Tmr_Channel) {
		case GPT_TIMER_0:
		...
		*TIMERS_INT_MASK_REG &= (0xFCU);
		switch (Config_Ptr->Op_Mode) {
			...
			*TIMERS_INT_MASK_REG |= ((Config_Ptr->Int_State)<<TIMER0_CMP_INT_BIT);
			return;
			case GPT_NORMAL_MODE:
			*TIMER0_CNTR_REG = 256U-(uint8_t)(Tmr_Counts);
			while (get_bit(*TIMERS_INT_FLAG_REG, TIMER0_OVF_INT_BIT) == 0) ;
			set_bit(*TIMERS_INT_FLAG_REG, TIMER0_OVF_INT_BIT);
			/* Re-set interrupt state */
			*TIMERS_INT_MASK_REG |= ((Config_Ptr->Int_State)<<TIMER0_OVF_INT_BIT);
		...
		*TIMERS_INT_MASK_REG &= (0xC3U);
		switch (Config_Ptr->Op_Mode) {
			case GPT_CTC_MODE:
			*TIMER1_CMP_A_REG_H = (uint8_t)(Tmr_Counts>>8U);
			*TIMER1_CMP_A_REG_L = (uint8_t)(Tmr_Counts & 0x00FFU);
			*TIMER1_CNTR_REG_H = 0x00U;
			*TIMER1_CNTR_REG_L = 0x00U;
			while (get_bit(*TIMERS_INT_FLAG_REG, TIMER1_CMP_A_INT_BIT) == 0) ;
			...
			*TIMERS_INT_MASK_REG |= ((Config_Ptr->Int_State)<<TIMER1_CMP_A_INT_BIT);
			return;
			case GPT_NORMAL_MODE:
			*TIMER1_CNTR_REG_L = (uint8_t)((65535U-Tmr_Counts) & 0x00FFU);
			*TIMER1_CNTR_REG_H = (uint8_t)((65535U-Tmr_Counts)>>8U);
			while (get_bit(*TIMERS_INT_FLAG_REG, TIMER1_OVF_INT_BIT) == 0) ;
			set_bit(*TIMERS_INT_FLAG_REG, TIMER1_OVF_INT_BIT);
			/* Re-set interrupt state */
			*TIMERS_INT_MASK_REG |= ((Config_Ptr->Int_State)<<TIMER1_OVF_INT_BIT);
		...
		*TIMERS_INT_MASK_REG &= (0x3FU);
		switch (Config_Ptr->Op_Mode) {
			...
			*TIMERS_INT_MASK_REG |= ((Config_Ptr->Int_State)<<TIMER2_CMP_INT_BIT);
			return;
			case GPT_NORMAL_MODE:
			*TIMER2_CNTR_REG = 256U-(uint8_t)(Tmr_Counts);
			while (get_bit(*TIMERS_INT_FLAG_REG, TIMER2_OVF_INT_BIT) == 0) ;
			set_bit(*TIMERS_INT_FLAG_REG, TIMER2_OVF_INT_BIT);
			/* Re-set interrupt state */
			*TIMERS_INT_MASK_REG |= ((Config_Ptr->Int_State)<<TIMER2_OVF_INT_BIT);
